...
const EmployeeMaintenanceEditMatInv = ({ userRole }) => {  // Use userRole here
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchMaterialInventory = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/material_inventory/${id}`); // Use userRole here
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:5000/api/material_inventory/${id}?role=${userRole}`, material_inventory);
...
export default EmployeeMaintenanceEditMatInv;
