...
const ManagerMaintenanceEditMatInv = ({userRole}) => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchMaterialInventory = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/material_inventory/${id}?role=${userRole}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:5000/api/material_inventory/${id}`, material_inventory);
...
export default ManagerMaintenanceEditMatInv;
