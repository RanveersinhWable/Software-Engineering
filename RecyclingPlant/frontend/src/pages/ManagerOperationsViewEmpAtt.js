...
const ManagerOperationsViewEmpAtt = ({ userRole }) => { // Pass userRole as a prop
    const [attendanceData, setAttendanceData] = useState([]);
    const [error, setError] = useState(null);

    useEffect(() => {
        // Fetch attendance records based on user role
        axios.get(`http://localhost:5000/api/employee_attendance/manager/${userRole}`) // Use userRole in the URL
            .then(response => {
    ...
    return (
        <div style={{ textAlign: 'center', marginTop: '50px' }}>
            ...
            <Link to="/manageroperations/attendance/add">
                <button style={{ padding: '10px 20px', marginBottom: '20px' }}>Add Attendance Record</button>
            </Link>
                    ...
                    {attendanceData.length > 0 ? (
                        attendanceData.map(record => (
                            <tr key={record.Attendance_ID}>
                                ...
                                <td>
                                    <Link to={`/manageroperations/attendance/edit/${record.Attendance_ID}`}>
...
export default ManagerOperationsViewEmpAtt;