...
app.get('/api/employee_attendance/manager/:role', (req, res) => {
    const { role } = req.params; // Extract user role from the route parameters
...
app.post('/api/employee_attendance/manager:role', (req, res) => {
    const {
        Attendance_ID,
        Employee_ID,
        Date: attendanceDate,
    ...
    db.query(getEmployeeDetailsQuery, [Employee_ID], (err, results) => {
        ...
        // Determine which stored procedure to call based on the Employee_Role
        let procedureName = '';
        if (Employee_Role === 'Manager-Operations') {
            procedureName = 'InsertAttendanceManagerOperations';
        } else if (Employee_Role === 'Manager-Maintenance') {
            procedureName = 'InsertAttendanceManagerMaintenance';
        } else {
            return res.status(400).json({ success: false, message: 'Invalid manager role.' });
        }

        // Call the respective stored procedure
        ...
        db.query(query, [
            ...
            calculateAttendanceRate(Employee_ID, isNotApplicable)
                .then(attendanceRate => {
                    res.json({ 
                        success: true, 
                        message: 'Employee attendance added successfully for manager.', 
                        attendanceRate,
                        departmentId: Department_ID 
                    });
                })
